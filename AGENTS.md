...
through `.ci/humble.sh`. The ROS2 job builds and runs the compat layer's parameter
mapping test - it does not fly the robots, because the MuJoCo models are generated by
the catkin build and are gitignored. Verify flight by hand.
