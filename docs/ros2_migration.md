...
`.github/workflows/ros2_test.yml` builds the ROS2 half on Ubuntu 22.04 / humble,
alongside the existing ROS1 job in `ros_test.yml`. It runs `.ci/humble.sh`, the
counterpart of `.travis.sh`; the two are kept separate because they share no
tooling - colcon against catkin, vcs against wstool, different dependency lists -
and the ROS1 script is what the flying robots' CI runs.

Source dependencies come from `aerial_robot_humble.rosinstall`, which is
deliberately shorter than the ROS1 list: that one also pulls livox_ros_driver2,
fast_lio, ublox_gps, rosserial, mocap_optitrack and sensor_interface, none of
which have a ROS2 build here and all of which colcon would try to build.

**The workflow cannot pass until the `ros2` branches of aerial_robot_3rdparty,
kalman_filter and mujoco_catkin are pushed.** They exist only locally today.
mujoco_ros_pkgs is fine - its `ros2` branch is identical to the pushed
`PR/hybrid-devel/depend_mujoco`, which is what the rosinstall names.

**The job generates the MuJoCo models**, as the catkin job does - they are
gitignored build products, so a fresh checkout has none and a job that skipped
...
## Generating the MuJoCo models

`mujoco_ros_utils` in aerial_robot_3rdparty turns a robot's xacro into a MuJoCo
model, and it now does that under both versions. Almost all of it is XML
manipulation with no ROS in it; four things were version-specific:

- **Where a package's directory is.** ROS1's rospack answers with the *source*
  directory, because a catkin devel space points back at it, and that is where
  these models have always been written. ROS2's ament index only knows the
  installed share directory, which is somewhere else and is replaced on every
  rebuild. So under ROS2 the caller passes it in: `--package-dir NAME=PATH`, and
  the cmake macro has it as `${PROJECT_SOURCE_DIR}`.
- **The MuJoCo compile tool**, which is the official binary the `mujoco` package
  wraps. Both versions install it as that package's executable, so only the
  launcher differs: `rosrun mujoco compile` against `ros2 run mujoco compile`.
- **xacro**, likewise: ROS2 puts it on PATH.
- **config/world.xml**, now found relative to the script under both versions,
  which is simpler and works before the package is installed.

One deliberate difference in the output. Under ROS1 the generated model includes
the world by a path relative to the source tree; under ROS2 the world is copied
in beside the model and included by name, because a ROS2 package is *installed*
somewhere else and a path out of the generated directory would not survive that.
Everything else in the generated XML is byte-identical - checked by generating
mini_quadrotor's model both ways and diffing.

**There is no cmake macro on the ROS2 side**, unlike catkin's
`mujoco_model_convert()`, and that is not an oversight. A robot's xacro says
`$(find <itself>)`, which under ROS2 resolves through the ament index and so
needs the package *installed* - it cannot generate its own model during its own
build. So generation is a separate step against a built workspace:

```
colcon build ...
source install/setup.bash
ros2 run mujoco_ros_utils mujoco_model_generator.py \
    src/jsk_aerial_robot/robots/hydrus/config/mujoco_model.yaml \
    --package-dir hydrus=src/jsk_aerial_robot/robots/hydrus
colcon build --cmake-force-configure --packages-select hydrus
```

`--cmake-force-configure` on the second build is not optional: whether a robot
package installs a model is decided at *configure* time, and the first pass
