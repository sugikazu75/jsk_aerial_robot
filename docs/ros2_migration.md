...
**The job does not fly the robots.** The MuJoCo models every simulation needs
are generated by `mujoco_ros_utils` during the *catkin* build and are gitignored,
so a humble-only checkout has none. The robot packages therefore treat a missing
model as a warning rather than an error - they still build, the simulation just
has nothing to load. Flying is verified by hand; see the takeoff check above.
Wiring the model generation into the ROS2 build, or committing the models, would
let CI fly them.
