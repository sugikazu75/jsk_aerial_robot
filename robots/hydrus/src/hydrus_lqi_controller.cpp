...
void HydrusLQIController::initialize(ros_compat::NodeHandle nh,
...
{
  ros_compat::SharedPtr<HydrusRobotModel> hydrus_robot_model =
    ros_compat::dynamicPointerCast<HydrusRobotModel>(robot_model_);
  ...
  if(!robot_model_->stabilityCheck(verbose_))
    {
      ...
      if(hydrus_robot_model->getWrenchDof() == 4 && hydrus_robot_model->getFeasibleControlRollPitchMin() > hydrus_robot_model->getFeasibleControlRollPitchMinThre())
        {
          ROS_COMPAT_WARN("LQI gain generator: change to three axis stable mode");
          lqi_mode_ = 3;
          return true;
        }
